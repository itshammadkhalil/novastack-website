...
const Footer = () => {
  ...
  const Modal = () => {
    return ReactDOM.createPortal(
      <div 
        className="fixed inset-0 z-[9999999] flex items-center justify-center p-4 bg-black/80"
      ...
      >
        ...
        >
          <button onClick={() => setModalContent(null)} className="absolute top-6 right-6 text-zinc-400 hover:text-white">
            <X size={24} />
          </button>
          ...
          <button onClick={() => setModalContent(null)} className="w-full py-3 bg-white text-black font-black uppercase text-[10px] rounded-xl hover:bg-green-500 transition-all">
            Close
          </button>
        </div>
      </div>,
  ...
  return (
    <>
      <footer className="w-full bg-zinc-950 border-t border-white/5 py-12 flex-shrink-0">
