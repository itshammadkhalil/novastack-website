...
const projects = [
  {
    ...
    link: "https://sardarscent.com" // Live Link Added
  },
  {
    ...
    link: "https://ibrahimwarsi.com" // Live Link Added
  },
  {
    ...
    link: "https://lahorivipburger.com" // Live Link Added
  }
];

const Portfolio = () => {
  return (
    <section id="work" className="relative w-full py-24 bg-zinc-950/50 flex flex-col items-center justify-center border-t border-zinc-900/50 overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
        ...
        <div className="text-center mb-16">
          ...
          >
            A glimpse into our recent successful collaborations. Real projects, real results.
          </motion.p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project, index) => (
            <motion.a // 'div' ko 'a' mein change kiya taake poora card clickable ho
              key={index}
              href={project.link}
              target="_blank"
            ...
            >
              <div className="relative h-48 sm:h-64 w-full overflow-hidden">
                <img 
                  src={project.image} 
                  alt={project.title} 
                  className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110 opacity-60 group-hover:opacity-100 grayscale-[50%] group-hover:grayscale-0"
                />
                ...
                <div className="absolute inset-0 bg-zinc-950/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center backdrop-blur-sm">
                  <div className="flex flex-wrap gap-2 justify-center px-4">
                    {project.techStack.map((tech, i) => (
                      <span key={i} className="px-3 py-1 bg-green-500/20 text-green-400 text-xs font-bold rounded-full border border-green-500/30">
                        {tech}
                      </span>
                    ))}
                  </div>
